...
while STATE is True:
    ...
    try:
        with open(
            "./files/" + FILE_NAME, "r", encoding="utf-8"
        ) as FILE_HANDLE:  # variable to access file
            ...
            for word in FILE_WORDS:  # loops over each word in list
                if (
                    word not in FILE_DICTIONARY
                ):  # checks if word ia already in dictionary
                    FILE_DICTIONARY[word] = (
                        1  # if not add the word as a key w/ value of 1
                    )
                else:
                    FILE_DICTIONARY[word] = (
                        FILE_DICTIONARY[word] + 1
                    )  # if in add 1 to count
